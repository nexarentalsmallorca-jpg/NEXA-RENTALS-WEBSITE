...
const AUTO_CAPTURE_SAMPLES = 2;
const CAMERA_WARMUP_MS = 600;
const QUALITY_CHECK_INTERVAL_MS = 140;
const MANUAL_CAPTURE_DELAY_MS = 1800;
...
export default function VerifyDocumentsPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      } else if (brightness > 250 || glareRatio > 0.55) {
        nextQuality = {
          tone: "warn",
          text: copy.tooMuchGlare,
        };
      } else if (contrast < 10 || edgeScore < (step === "dlBack" ? 4.6 : 3.8)) {
        nextQuality = {
          tone: "warn",

...
export default async function ScooterRentalGuidesPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#F9F8F7] text-stone-950">
      <Navbar />
        ...
        <section className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {blogs.map((blog) => (
            <Link
              ...
              className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-md"
            >
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-stone-400">
                {blog.category} · {blog.readTime}
              </p>

              <h2 className="mt-3 text-lg font-semibold leading-snug text-stone-950">
                {blog.title}
              </h2>
